 ...
 */
package com.quantfabric.algo.cep.indicators.laguerrefilter.test;
...
import com.quantfabric.algo.cep.indicators.laguerrefilter.LaguerreFilterPluginViewFactory;
...
public class LaguerreFilterTestApp {
	...
	{
		configuration.addPlugInView("quantfabric", "lagfilter", LaguerreFilterPluginViewFactory.class.getName());		
	}
	...
	private static void registerTestStatements(EPAdministrator epAdministrator)
	{
		...
		epAdministrator.createEPL("select * from Bars(closed=true).quantfabric:lagfilter(barId, 0.7, close)").addListener(
			new UpdateListener() 
			{				
				@Override
