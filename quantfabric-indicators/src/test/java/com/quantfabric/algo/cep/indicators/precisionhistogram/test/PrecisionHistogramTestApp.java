 ...
 */
package com.quantfabric.algo.cep.indicators.precisionhistogram.test;
...
import com.quantfabric.algo.cep.indicators.precisionhistogram.PrecisionHistogramPlugInView;
import com.quantfabric.algo.cep.indicators.precisionhistogram.PrecisionHistogramPlugInViewFactory;

public class PrecisionHistogramTestApp {
	...
	{
		configuration.addPlugInView("quantfabric", "psh", PrecisionHistogramPlugInViewFactory.class.getName());		
	}
	
	private static void registerTestStatements(EPAdministrator epAdministrator)
	{
		epAdministrator.createEPL("insert into Bars select * from InputBean.quantfabric:ohlc('1 min', timestamp, price) as OHLCValue"); 
		
		epAdministrator.createEPL("select * from Bars.quantfabric:psh(barId, close, closed)").addListener(
			new UpdateListener() 
			{				
				@Override
	...
	public static void main(String[] args) throws InterruptedException
	{
		...
		PrecisionHistogramPlugInView.getLogger().setLevel(Level.ALL);
