...
public class CEPProvider {
    ...
    public static ICEPProvider getCEPProvider(String uri,Configuration config) {
    	return new EsperCEPProvider(uri, config);
    }
    
    public static ICEPProvider getCEPProvider(String uri,Configuration config,
    		Collection<PersistingUpdateListenerConfig> persisterConfigs) {
    	return new EsperCEPProvider(uri, config, persisterConfigs);
    }
}