...
public class XChangeOrderExecutionState {
	
	public enum XChangeExecutionState {
		Accepted,
		Filled,
		PartiallyFilled,
		Cancelled,
		...
		public boolean equals(Object obj) {
			...
			if (obj instanceof PartiallyFilledOrder) {
				...
				return this.orderId.equals(comparedOrder.getOrderId()) &&
						...
						new BigDecimal(this.tradeAmount * XChangeConfirmationBatch.POINTS_IN_ONE).longValue() == comparedOrder.getTradeAmount();
			}
	...
	private XChangeExecutionState executionState;
	...
	private final List<PartiallyFilledOrder> partialFills = new ArrayList<>();
	private final Object lock = new Object();
	public static final int SIZE_CORRECTOR = 100;
	
	public XChangeOrderExecutionState(TradeOrder tradeOrder, String marketOrderReference, boolean isOrderRejected) {
		setTradeOrder(tradeOrder);
		setMarketOrderReference(marketOrderReference);
		if (isOrderRejected)
			setExecutionState(XChangeExecutionState.Rejected);
		else
			setExecutionState(XChangeExecutionState.Unknown);
		leavesQty = (long) tradeOrder.getSize() * XChangeConfirmationBatch.POINTS_IN_ONE / SIZE_CORRECTOR;
	}
	...
	public XChangeExecutionState getExecutionState() {
		return executionState;
	}

	public void setExecutionState(XChangeExecutionState executionState) {
	...
	public double getAvgPx() {
		...
		if (!isPartiallyFilled())
			...
			synchronized (lock) {
				...
				if (isCompleted) {
					VWAP += getTrade().getPrice().doubleValue() * Math.abs(trade.getOriginalAmount().doubleValue() * XChangeConfirmationBatch.POINTS_IN_ONE);
					totalQuantity += Math.abs(trade.getOriginalAmount().doubleValue() * XChangeConfirmationBatch.POINTS_IN_ONE);
				}
	...
	public double getQuantity() {
		...
		if (!isPartiallyFilled())
			...
			synchronized (lock) {
				...
				if (isCompleted)
					cumulativeQuantity += Math.abs(trade.getOriginalAmount().doubleValue() * XChangeConfirmationBatch.POINTS_IN_ONE);
				
				return cumulativeQuantity / XChangeConfirmationBatch.POINTS_IN_ONE;
			}
		}
	}
}
